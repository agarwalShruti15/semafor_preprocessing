...
# general
parser.add_argument('--input_video', type = str, default='M2pi9JUuhBA.mp4', help='input video')
parser.add_argument('--output_video', type = str, default = 'M2pi9JUuhBA_out.mp4', help = 'output video')
...
parser.add_argument('--chunk_size', default=8, type=float, help='chunk size, reduce it if out of memory')
...
for i in tqdm(range(0, len(list_pathfile_img), args.chunk_size)):
	imgs = []
	imgs_crops = []
	imgs_bbox = []
	img_a = []
	for j in range(i, min(len(list_pathfile_img), i+args.chunk_size)):
		img_whole = cv2.imread(list_pathfile_img[j])
		black = img_whole.copy()
		black[:] = 0
		cv2.imwrite(os.path.join('output', os.path.basename(list_pathfile_img[j])), black)
		img_crops, img_mat, img_bbox = app.get_with_bbox(img_whole,224)
		imgs.append(img_whole)
		imgs_crops.append(img_crops)
		imgs_bbox.append(img_bbox)
		if img_crops == None:
			continue
		for k in range(len(img_crops)):
			img_a.append(transformer_Arcface(img_crops[k]))
	if all(v is None for v in imgs_crops):
		continue
	img_id = torch.stack(img_a)
	img_id = img_id.cuda()
	...
	for j in range(len(imgs)):
		if imgs_crops[j] == None:
			continue
		if len(imgs_crops[j]) == 1:
			idx = idx+1
			for ref_f in ref_feat:
				if np.dot(latend_id[idx-1], ref_f) > args.threshold:
					cv2.imwrite(os.path.join('output', os.path.basename(list_pathfile_img[i+j])), imgs[j])
					break
		else:
			for k, feat in enumerate(imgs_crops[j]):
				idx = idx+1
				for ref_f in ref_feat:
					if np.dot(latend_id[idx-1], ref_f) > args.threshold:
						bbox = imgs_bbox[j][k,0:4]
						x = int(bbox[0])
						w = int(bbox[2]) - x
						y = int(bbox[1])
						h = int(bbox[3]) - y
						...
						if y+h > black.shape[0]-1:
							h = black.shape[0]-1 - y
						nimg = black.copy()
						nimg[y:y+h, x:x+w] = imgs[j][y:y+h, x:x+w]
						cv2.imwrite(os.path.join('output', os.path.basename(list_pathfile_img[i+j])), nimg)
						break
...
if has_audio:
    command = 'ffmpeg -y -hide_banner -loglevel error -framerate {} -i output/%08d.jpg -i temp.wav -c:v libx264 -preset veryslow -crf 18 -pix_fmt yuv420p {}'.format(fps, args.output_video)
else:
    command = 'ffmpeg -y -hide_banner -loglevel error -framerate {} -i output/%08d.jpg -c:v libx264 -preset veryslow -crf 18 -pix_fmt yuv420p {}'.format(fps, args.output_video)
